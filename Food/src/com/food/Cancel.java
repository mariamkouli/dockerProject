	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			//3- process
			 ...
			 out.println(HTML_START+"<form action='ProcessCancel' method='GET' id='searchfield'>"+"<TABLE>" );

			while (res.next()) {
				 out.println("<TR>\r\n" + 
					 		"<TD>\r\n"+"<input type='checkbox' NAME='meal' value ='"+res.getString("day" )+"'/>"+"</TD>\r\n<TD>");
				 out.println(res.getString("day" )+"</TD>\r\n"+  
					 		"<TD>");
				 out.println(res.getString("meal" )+"</TD>\r\n"+  
				 		"</TR>");
			}
			out.println(" <input type='hidden' name='studentNumber' value='"+request.getParameter("studentNumber")+"'/>" );
			out.println("</TABLE>\r\n"+"<input type='submit' value = 'Drop' /></form>"+HTML_END);
		} catch (Exception e) {
